...
app.MapGet("/api/campsites/{id}", async (CreekRiverDbContext db, int id) =>
{
    try
    {
        var site = await db.Campsites.Include(c => c.CampsiteType).SingleAsync(c => c.Id == id);

        // If the site is found, return an "OK" response.
        return Results.Ok(site);
    }
    catch (InvalidOperationException)
    {
        // If no campsite is found, return a "Not Found" response.
